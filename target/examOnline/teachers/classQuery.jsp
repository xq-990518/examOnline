        ...
        <div class="maincontent">
            <div class="maincontentinner">
                ...
                <table id="dyntable" class="table table-bordered responsive">
                    ...
                    <tr align="center">
                        <th class="head0" width="20%">班级编号</th>
                        <th class="head1" width="20%">班级名称</th>
                        <th class="head0" width="20%">班级人数</th>
                        <th class="head1" width="20%">查看所有学生</th>
                        <th class="head0" width="20%">查询成绩</th>
                    </tr>

                    </thead>
                    <tbody>
                    <c:forEach var="tclass" items="${page.topicList}">
                        <c:forEach var="tea" items="${teacher.tClasses}" varStatus="index">
                            <%--判断两个班级的id是否相同，若是相同则显示班级--%>
                            <c:if test="${tea.c_id==tclass.c_id}">
                                <tr align="center">
                                    <td>${tclass.c_id}</td>
                                    <td align="left"> ${tclass.c_name} </td>
                                    <td align="left">
                                            <%--遍历学生--%>
                                        <c:forEach items="${tclass.students}" var="StuNum" varStatus="index">
                                            <%--如果此时是遍历的最后一行，则显示此时的行数（既人数）--%>
                                    ...
                                    <td>
                                        <a href="<%=path%>/ShowAllStudentByClass?t_id=${teacher.t_id}&subject_id=${teacher.subject.subject_id}&c_id=${tclass.c_id}&pageNo=0">查看</a>
                                    </td>
                                    <td>
                                        <a href="<%=path%>/teachers/ExamScoreQuery.jsp?t_id=${teacher.t_id}&c_id=${tclass.c_id}">查看</a>
                                    </td>
                                </tr>
                            </c:if>
                        </c:forEach>
                    ...
                    <tr>
                        <td colspan="6" align="center">
                            ...
                            <a href="showAllTClass?t_id=${teacher.t_id}&pageNo=1">首页</a> |
                            <a href="showAllTClass?t_id=${teacher.t_id}&pageNo=${page.currentPageNO - 1}">上一页</a> |

                            <a href="showAllTClass?t_id=${teacher.t_id}&pageNo=${page.currentPageNO + 1}">下一页</a> |
                            <a href="showAllTClass?t_id=${teacher.t_id}&pageNo=${page.totalPageCount}">尾页</a>
